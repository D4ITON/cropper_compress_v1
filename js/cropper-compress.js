 ...
 *
 * Date: Tue May 02 13:13:03 2019 GMT-05
 */
...
window.onload = function () {
   ...
   */
  btnGoBack.style.display = docs_buttons.style.display = docs_toggles.style.display = docs_advanced.style.display =
    "none";
      ...
      if (cropper && files && files.length) {
        ...
        if (/^image\/\w+/.test(file.type)) {
          ...
          reader.onloadend = function () {
            ...
            docs_toggles.style.display = docs_buttons.style.display = docs_advanced.style.display =
              "block";
  ...
  activeAdvanced.onclick = async function () {
    ...
    var activeAdvancedMessage = document.querySelector(
      "#activeAdvancedMessage"
    );
  ...
  actions.querySelector(".docs-toggles").onchange = function (event) {
    ...
    return (cropper = new Cropper(
      arrayImages[arrayImages.length - 1],
      options
    ));
  };
  ...
  btnCancel.onclick = function () {
    ...
    if (btnSaveUpload.innerHTML === "Cortar") {
      btnSaveUpload.innerHTML = "Descargar";
  ...
  function muestraCaracteristicas(pesoFinalSize) {
    ...
    let variacionPeso = Math.abs(
      100 - parseInt((100 * pesoFinalSize) / pesoInicialSize)
    );
